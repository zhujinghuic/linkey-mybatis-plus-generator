...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.ENUM, joinPackage(config.getParent(), config.getEnumName()));
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getEnumName(getGlobalConfig().isKotlin()), outputDir, ConstVal.ENUM_PATH, ConstVal.ENUM);
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            tableInfo.setEnumName(entityName + "Enum");
            // 检测导入包
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, StrategyConfig config) {
        ...
        List<TableEnum> enumList = new ArrayList<>();
        ...
        try {
            ...
            try (
                ...
                while (results.next()) {
                    ...
                    String fieldComment = results.getString(dbQuery.fieldComment());
                    TableEnum enumField = getTableEnum(fieldType, columnName, fieldComment);
                    ...
                    fieldList.add(field);
                    if (StringUtils.isNotBlank(enumField.getName())) {
                        enumList.add(enumField);
                    }
                }
            }
        } catch (SQLException e) {
        ...
        tableInfo.setEnums(enumList);
        tableInfo.setCommonFields(commonFieldList);
        return tableInfo;
    }

    /**
     * 加入带指定数据类型和特定格式的注释字段为枚举
     * @param fieldType
     * @param columnName
     * @param fieldComment
     * @return
        ...
        if (fieldType.contains("tinyint") && accordCommentFormat(fieldComment)) {
            try {
                ...
                for (String field : fields) {
                    ...
                    enumField.setValue(keyValues[1].trim());
                    enumField.setName(columnName.toUpperCase() + "_" + keyValues[0].trim());
                    enumFields.add(enumField);
                }
                tableEnum.setEnumFields(enumFields);
    ...
    private boolean accordCommentFormat(String fieldComment) {
        if (Pattern.compile("[{\\：\\，\\}]+").matcher(fieldComment).find()) {
            return true;
        }
        return false;
    }
